...
class WatermarkRemoverService:
    ...
    def _texture_reconstruct(self, image, mask):
        patch_size = self._env_int(
            "WATERMARK_PATCH_SIZE",
            15,
            5,
            51,
        )
        ...
        search_radius = self._env_int(
            "WATERMARK_SEARCH_RADIUS",
            80,
            20,
            300,
        )

        iterations = self._env_int(
            "WATERMARK_PATCH_ITERATIONS",
            2,
            1,
            5,
        )
    ...
    def _blend_result(self, original, reconstructed, mask):
        blend_size = self._env_int(
            "WATERMARK_BLEND_SIZE",
            5,
            1,
            21,
        )

        kernel = np.ones((blend_size, blend_size), np.uint8)
        soft_mask = cv2.GaussianBlur(
            mask,
            (blend_size | 1, blend_size | 1),
            0,
        ...
        result = (
            reconstructed.astype(np.float32) * soft_mask
    ...
    def remove(self, image_bytes, strokes):
        ...
        dilation_size = self._env_int(
            "WATERMARK_MASK_DILATE_SIZE",
            3,
            1,
            15,
        )
        ...
        reconstructed = self._texture_reconstruct(
            image_cv,
            mask,
        )
        ...
        missing = np.any(reconstructed == image_cv, axis=2)
        reconstructed[missing & (mask > 0)] = fallback[
            missing & (mask > 0)
        ]

        result = self._blend_result(
            image_cv,
            reconstructed,
            mask,
        )
